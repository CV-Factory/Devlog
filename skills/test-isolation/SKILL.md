...
- **테스트 설계 방법론 (필수)**:
    - **EP-BVA (등가 분할 & 경계값 분석)**: 입력값의 유효/무효 범위를 나누고, 경계값(최소, 최대, 경계 직전/직후)을 반드시 포함한다.
    - **Pairwise (페어와이즈)**: 여러 입력 변수의 조합이 많을 경우, 모든 쌍을 최소 1회 이상 조합하여 테스트 케이스를 최적화한다.
    - **State Transition (상태 전이)**: 객체나 시스템의 상태 변화가 중요한 경우, 상태 전이도 기반으로 모든 상태와 전이 경로를 검증한다.
- **비대화형 테스트 실행**: 모든 테스트 명령어는 사용자 입력 대기 없이 즉시 결과를 반환하도록 실행한다. (Watch 모드 사용 금지)
- **국소적 돌연변이 테스트**: 돌연변이 테스트(Mutation Testing) 수행 시 전체 코드베이스를 한꺼번에 하지 않고, 대상 코드를 국소적으로 나누어 진행한다.
